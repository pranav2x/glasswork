  ...
  if (isAuthLoading || analyses === undefined) {
    return (
      <div className="space-y-6 p-6">
        <div className="h-10 w-64 rounded-xl bg-white/[0.04] shimmer-bg" />
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="h-[180px] rounded-2xl bg-white/[0.04] shimmer-bg" />
          ))}
        </div>
      </div>
    );
            ...
            <button
              ...
              className={`rounded-full px-4 py-1.5 text-[12px] font-medium transition-all duration-200 ${
                activeFilter === f.value
                  ? "bg-brand/15 text-brand border border-brand/20 shadow-[0_0_12px_rgba(124,111,255,0.15)]"
                  : "bg-white/[0.04] text-warm-500 hover:bg-white/[0.08] hover:text-warm-700 border border-white/[0.08]"
              }`}
            >
              {f.label}
            </button>
        ...
        {filtered.length === 0 ? (
          <div className="hero-fade-in flex flex-col items-center justify-center rounded-2xl border border-dashed border-white/[0.12] bg-white/[0.02] py-20" style={{ animationDelay: "0.08s" }}>
            <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-white/[0.06]">
              <Plus className="h-6 w-6 text-warm-500" strokeWidth={1.5} />
            </div>
            ...
            {activeFilter === "all" && (
              <button
                ...
                className="mt-5 rounded-xl bg-brand px-5 py-2.5 text-[13px] font-semibold text-white shadow-glow-brand transition-all hover:bg-brand-light hover:scale-105 active:scale-95"
              >
                New Analysis
              </button>
            )}
