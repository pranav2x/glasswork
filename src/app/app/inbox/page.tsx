...
const NOTIFICATION_COLORS = {
  analysis_complete: "text-solid bg-solid/10",
  mvp_status: "text-carry bg-carry/10",
  tier_change: "text-warm-600 bg-white/[0.06]",
  streak_milestone: "text-amber-400 bg-amber-400/10",
} as const;
    ...
    return (
      <div className="space-y-6 p-6">
        ...
        {Array.from({ length: 4 }).map((_, i) => (
          <div key={i} className="h-[80px] rounded-2xl bg-white/[0.04] shimmer-bg" />
        ))}
      </div>
    );
  }
      ...
      <div className="space-y-6 p-6">
        ...
        <div className="hero-fade-in flex items-end justify-between">
          ...
          {unreadCount > 0 && (
            <button
              onClick={() => markAllRead({})}
              className="flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-[12px] font-medium text-warm-500 transition-colors hover:bg-white/[0.06] hover:text-warm-700"
            >
        ...
        {notifications.length === 0 ? (
          <div className="hero-fade-in flex flex-col items-center justify-center rounded-2xl border border-dashed border-white/[0.12] bg-white/[0.02] py-20" style={{ animationDelay: "0.08s" }}>
            <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-white/[0.06]">
              <Inbox className="h-6 w-6 text-warm-500" strokeWidth={1.5} />
            </div>
          ...
          <div className="hero-fade-in space-y-2" style={{ animationDelay: "0.04s" }}>
            {notifications.map((n) => {
              ...
              const colorClass = NOTIFICATION_COLORS[n.type] || "text-warm-500 bg-white/[0.06]";
                ...
                <div key={n._id} className="relative">
                  ...
                  >
                    <GlassPanel className={cn(
                      "flex items-start gap-4 p-4 pr-12 transition-all hover:-translate-y-0.5 hover:shadow-glass-hover",
                      !n.read && "border-brand/20"
                    )}>
                      {!n.read && (
                        <div className="absolute left-2 top-1/2 -translate-y-1/2 h-2 w-2 rounded-full bg-brand" />
                      )}
                  ...
                  <button
                    ...
                    className="absolute right-3 top-1/2 -translate-y-1/2 flex h-7 w-7 items-center justify-center rounded-lg text-warm-400 transition-colors hover:bg-white/[0.06] hover:text-danger"
                    title="Delete"
                  >
